...
if(isset($_POST["question"])){
      ...
      if ($result["RESULT"] == 1) {
        ...
        $data = array("RESULT"=> 1, "MESSAGE" => "nuke_question was successfully asked!", "user_id" => $questionID);
      } else {
        if (strpos($result["MESSAGE"], 'Duplicate entry') !== false) {
            $data = array("RESULT"=> 2, "MESSAGE" => "Duplcate question asked!");
        } else {
            $data = array("RESULT"=> 2, "MESSAGE" => "nuke_question was not saved. Please try again. If this problem reoccurs, please contact support.");
        }
      }
